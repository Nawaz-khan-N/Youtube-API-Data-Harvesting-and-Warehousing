...
#c = 0
#st.text("Type YES and Enter button")
...
st.text("Youtube Data Harvesting & Warehousing")
x = st.text_input("Enter the Channel Name")
if st.button("Retrive"):
    y = getChannel(x)
    st.write(y)
    mn = st.button("Insert to MongoDB")
    st.write(mn)
    if mn:
        col.insert_one(y)
        ...
        if st.button("Display"):
            ...
            if st.button("Insert to SQL"):
                ...
                if st.button("Retrive Data"):
                    z = st.text_input("Enter the Query")
                    op = pd.read_sql_query(sql=text(z), con=engine.connect())
                    st.write(op)